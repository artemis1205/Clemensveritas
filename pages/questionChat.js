...
export default function QuestionChat(props) {
    ...
    useEffect(() => {
        ...
        gun.get(props.chatToken + 'Q').map().once(function (m) {
    ...
    function saveMessage() {
        ...
        const messages = gun.get(props.chatToken + 'Q');
        if (form != "") {
            messages.set({
                name: username,
                titel: form.titel,
            ...
            if (check == true) {
                ...
                gun.get(props.chatToken + 'Q').map().once(function (m) {
                    ...
                    if (m._['#'] === k) {
                        ...
                        gun.get(props.chatToken + 'Q').get(`${k}`).put({ likeAnz: likeM + 1 })
                    }
                })
            }
            console.log(check)
    ...
    return (
        <>
            ...
            {replie ? (
                <>
                    <Reply back={replySwitch} keys={key} chToken={props.chatToken + 'Q'} />
                </>
            ) : (
                    <>
                        <div >
